...
class _LiveStockDetailViewState extends State<LiveStockDetailView> {
  ...
  Future<void> _getAIRecommendations() async {
    ...
    try {
      final response = await _livestockAdviceService.getLiveStockRecommendations(
        livestockName: _livestockNameController.text,
        livestockType: livestockProvider.selectedLivestockType!.name,
        livestockPurpose: livestockProvider.selectedLivestockPurpose!.name,
        gender: livestockProvider.selectedGender!.name,
            ...
            children: [
              if (!aiProvider.showAiResponse) ...[
                ...
                reusableDropdown<LivestockType>(
                  ...
                  selectedValue: livestockProvider.selectedLivestockType ?? LivestockType.cow,
                ...
                reusableDropdown<LivestockPurpose>(
                  ...
                  selectedValue: livestockProvider.selectedLivestockPurpose ?? LivestockPurpose.breeding,
                ...
                reusableDropdown<Gender>(
                  label: "Gender",
                  selectedValue: livestockProvider.selectedGender ?? Gender.male,
                  items: Gender.values,
                  onChanged: (newValue) {
                        ...
                        onClick: () {
                          if (_formKey.currentState!.validate()) {
                            ...
                            final livestock = LiveStockDetail(
                              id: _livestockDetails?.id ?? const Uuid().v4(),
                              liveStockName: _livestockNameController.text,
                              liveStockType: livestockProvider.selectedLivestockType?.name ?? 'cow',
                              purpose: livestockProvider.selectedLivestockPurpose?.name ?? 'breeding',
                              gender: livestockProvider.selectedGender?.name ?? 'male',
                              age: int.tryParse(_ageController.text) ?? 0,
                              vaccinatedDate: parseDate(_vaccinationDateController.text) ?? DateTime.now(),
                              aiRecommendations: aiProvider.aiResponse.isEmpty ? null : aiProvider.aiResponse,
                            );

                            if (widget.livestockId != null) {
                              livestockProvider.updateLivestock(_firestoreService.userId, livestock).then((_) {
                                showCustomSnackBar(context, "Livestock updated successfully");
                                Navigator.pop(context);
                              }).catchError((e) {
                                showCustomSnackBar(context, "Error updating livestock: $e");
                              });
                            } else {
                              livestockProvider.addLiveStock(livestock).then((_) {
                                showCustomSnackBar(context, "Livestock added successfully");
                                Navigator.pop(context);
                              }).catchError((e) {
                                showCustomSnackBar(context, "Error adding livestock: $e");
                              });
                            }
                          }
                        },
                        buttonName: widget.livestockId == null ? 'Save Details' : 'Update Details',
                      ),
